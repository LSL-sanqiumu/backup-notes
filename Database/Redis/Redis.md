...
4. 替换值：（位置下标从0开始）
   - `setrange key 1 "替换值"`：从某位置开始替换字符串（包括指定位置），不存在的key的value将会当作空白字符。

**值的获取：**（获取整个key的值、获取key的部分值）

- **`get key`。**
- **`mget k1 k2 k3 ...`：一次获取多个值。**
- `getrange key 0 3`：获取指定区间的字符串，是闭区间。（`getrange key 0 -1`：查看当前key，和`get key`的作用一样。）
...
3. 删除：
   - **`hdel myhash field  [field  ...]`：**删除myhash 的值中的某个属性和属性值。
5. 判断：
...
4. **`lrem list count value`：**从列表 list 中删除前 count 个值等于 element 的元素。
...
- `smove myset targetset value `：移动(剪切)myset中的value到targetset，如果targetset不存在会自动创建。
...
绑定redis服务器网卡IP，默认为127.0.0.1，即本地回环地址。这样的配置下，访问redis服务只能通过本机的客户端连接，而无法通过远程连接。如果bind选项为空的话，那会接受所有来自于可用网络接口的连接。如上配置，绑定一个127.0.0.1的本机地址和192.168.1.100的外网地址，这样的话主机192.168.1.100也能访问。
...
保护模式，默认是开启状态，只允许本地客户端连接，要允许外网访问就设置为no。
