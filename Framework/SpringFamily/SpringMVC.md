...
**1、@PathVariable：**用于接收路径变量，常是restful风格的请求地址，例如：
...
**2、@RequestParam：**用于接收get方法、post方法的请求携带的参数。
...
**3、@Validated：**@Validated注解可以用一个模型来接收地址栏中的参数（将请求参数封装进pojo类）。
...
**@RequestBody注解：**用来接收request的body中的参数，@RequestBody可以将多个参数放入到一个实体类或者Map中。（不能通过GET方法发送请求体，post请求提交的参数就放在请求体中）
...
**1、@RequestHeader 注解：**可以把请求头部分的值绑定到方法的参数上。
...
**2、@CookieValue：**把Request header中关于cookie的值绑定到方法的参数上。
...
// 浏览器页面显示（中文出现乱码）：success:??
```

@ResponseBody注解可以将请求方法返回的对象直接转换成JSON对象，但是当返回值是String的时候，中文会乱码，原因是因为其中字符串转换和对象转换用的是两个转换器，而String的转换器中固定了转换编码为"ISO-8859-1"；解决方法如下：

- @RequestMapping改为`@RequestMapping(value = "***",produces={"text/html;charset=UTF-8;","application/json;"})`。
...
![](imgs/spring-imgs/springMVC执行流程图.png)
