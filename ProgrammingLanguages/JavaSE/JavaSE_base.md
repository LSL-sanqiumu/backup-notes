...
/*【注意】：
    	...
    	关于自增和自减：只能作用于变量（用于变量命前后使用）
    	（前置时自身先加1，后置时自身先去被使用一次再进行加一；自增、自减不会改变自身数据类型。）
...
浮点型变量不能用==或!=来进行比较，如果需要比较则可以转换为>=或<=的形式来进行比较。
...
[Java 重写 hashCode 方法总结 - 知乎 (zhihu.com)](https://zhuanlan.zhihu.com/p/85423244#:~:text=Java 重写 hashCode 方法总结 1 若 x.equals (y),hashCode 方法返回的散列码总体上呈均匀分布，可以提高哈希表的性能。 3 程序运行时，若对象的 equals 方法中使用的字段没有改变，则在程序结束前，多次调用 hashCode 方法都应返回相同的散列码；程序结束后再执行时则没有此要求。)
...
3、JDK5.0新特性：
  ...
  - 自动拆箱：包装类对象可以直接赋给相应基本数据类型变量，包装类 ===> 基本数据类型。
