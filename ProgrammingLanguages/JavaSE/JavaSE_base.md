...
5. 数组遍历：循环、foreach循环、Arrays工具类的toString()方法（将数组以字符形式输出）。
...
2.实例化非静态内部类：得先去创建外部类的对象，再通过外部类对象创建内部类的对象
...
<img src="images/exception.png" style="zoom: 80%;" />
...
垃圾回收的GC(Garbage Collection)算法决定什么时候回收垃圾，可通过System.gc()主动触发垃圾回收。
