...
有符号数的原码、补码、反码：（**使用二进制表示数时都表示的是数的补码，八位补码0b1000000没有对应的原码，其表示-128**）
...
**位运算符：（操作的都是整型数据，顾名思义是在位上的运算，注意运算的时候是以补码的形式来进行运算，最后结果还要转换为原码，计算机内的数都是补码，如果赋值时直接赋值二进制数，这个二进制数也是补码）**

- <<        有符号左移，移出去的丢失（包括符号位），空出来的都补0。
- \>>        有符号右移，移出去的丢失，空出来的——原始数据最高位是啥就拿啥补。
- \>>>      无符号右移，符号位不参与的右移：移动时空出来的位都用0补，符号位不变。
- ~          取反运算：0 ===> 1，1 ===> 0。
...
权限修饰符是为了控制其他代码对这段代码的访问级别（决定哪里的代码才可以访问这段代码）。类的访问控制符（权限修饰符）只能是缺省或者 public，方法和属性的访问控制符有 4 个，分别是 public、 private、protected 和 缺省（friendly），其中缺省是一种没有定义专门的访问控制符的默认情况。
...
- ```java
  public class MethodTest {
      public static void main(String[] args) {
          // 可变形参参数传递的两种方式，public void show(数据类型[]  变量名){ }则只能传数组
          test(1,2,3,4);
          test(new int[44]);
      }
...
- 基本数据类型的形参传递的是传入的变量的值（将自己的值赋给形参）。
...
1. 继承：一个类可以作为子类来继承父类，使得子类有父类的数据或行为方法。继承可以方便功能拓展、提高代码的复用性，也为多态的使用提供了前提，继承是对代码的复用。（子类继承父类后就获得了父类的所有结构（属性和方法））（默认继承的只有属性和方法，构造方法不会被继承需要使用super调用；子类中会含有父类的所有的信息）。
...
匿名实现类就是直接使用接口充当构造，没有显式的新建实现类；匿名对象就是对象没有名字标识。
...
- Object类的方法，默认返回全类名+@+十六进制的哈希值。
- 子类重写`toString()`方法以实现显示属性值。
...
1. 基本数据类型 ---> 包装类：调用包装类的构造器，注意Boolean的构造器有些优化；
...
public void test() {
    ...
    // 如果我们用自动装箱的方式给Integer赋值的范围时可以直接使用数组中的元素，就不会再去new了
    // 目的：提高效率
...
我们在平常的代码中，都常用`System.out.print()`来放在程序中某个位置来测试、检测程序运行情况，但使用这种方式后面还得一步步删去这些代码，所以为了开发的方便，就提供了日志的功能来方便检测程序的运行状况，而且还能随意的确定是否开启日志。（日志就是为了解决使用`System.out.print()`来测试后得全部删除这些语句的问题而设计的）。
...
UML类图：用来表示类、接口或枚举类等的成员结构的一种结构示意图。
